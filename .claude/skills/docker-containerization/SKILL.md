...
**Novice**: Relies on `depends_on` alone; that only waits for the container to START, not for the service to be READY.
...
**Principle:** Docker HEALTHCHECK answers "Is this process alive?" not "Are all dependencies reachable?" Checking DB connectivity here causes cascading restarts when the DB recovers. Keep health checks cheap and local.

### Quick reference

| Service type | `test` command | `interval` | `start_period` |
|---|---|---|---|
| API (HTTP) | `wget -qO- http://localhost:${PORT}/health` | 30s | 10s |
| Worker (no HTTP) | `test $(find /tmp/worker-heartbeat -mmin -1 \| wc -l) -gt 0` | 30s | 15s |
| Static (nginx/Caddy) | `wget -qO- http://localhost:80/` | 30s | 5s |
| PostgreSQL | `pg_isready -U postgres` | 10s | 30s |
| Redis | `redis-cli ping` | 10s | 5s |
| MySQL | `mysqladmin ping -h localhost` | 10s | 30s |

Use `timeout: 3-5s`, `retries: 3` for apps, `retries: 5` for databases.

### Annotated API example
...
```js
// /health must: return 200, skip DB checks, respond in <100ms
app.get('/health', (req, res) => res.status(200).json({ status: 'ok' }));
// Put DB/queue status on /health/detailed; never wire to HEALTHCHECK
```

**Worker heartbeat pattern:** write `/tmp/worker-heartbeat` on each job loop iteration; health check fails if file is older than 1 minute (process stuck or event loop blocked).

---

## Emulated Builds (QEMU on Self-Hosted arm64 Runners)

When building `linux/amd64` images on an Apple Silicon (arm64) GitHub Actions runner using QEMU emulation, V8's TurboFan JIT tier causes SIGSEGV crashes during `next build` and similar compute-heavy steps.

**Do not use `--jitless`.** It disables WebAssembly entirely, which breaks Next.js/SWC (`ReferenceError: WebAssembly is not defined`).

**Correct fix: `--require` preload script calling `v8.setFlagsFromString`.**

```js
// scripts/disable-turbofan.cjs
require("v8").setFlagsFromString("--no-turbofan");
```

Wire it into the Docker build stage via `NODE_OPTIONS`:

```dockerfile
FROM node:22-alpine AS build
# Inject the preload for all node invocations, including Next.js workers
ENV NODE_OPTIONS="--require /app/scripts/disable-turbofan.cjs"
COPY scripts/disable-turbofan.cjs ./scripts/disable-turbofan.cjs
# ... rest of build stage
```
...
- `references/multi-stage-patterns.md`: complex multi-stage builds, build caching with BuildKit, cross-compilation, monorepo Dockerfiles, Bun/Deno patterns
