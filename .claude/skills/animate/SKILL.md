...
- `references/framer-motion.md` - Motion components, `AnimatePresence`, variants, layout animations, hooks (uses `motion` package, import from `motion/react`)
...
For Motion examples, install the `motion` package (successor to framer-motion):
```bash
pnpm add motion react-use-measure usehooks-ts
```
